...
* 3 functions, however, update existing tables in the database with new data from various [external API data sources](https://documenter.getpostman.com/view/10808728/SzS8rjbc?version=latest). Each day a CloudWatch rule triggers the 2 functions to parse the data from the external APIs, updating the [summary](https://github.com/Lambda-School-Labs/earth-dashboard-ds/blob/master/AWSLambda/summary_db_add/lambda_function.py) and [covidall](https://github.com/Lambda-School-Labs/earth-dashboard-ds/blob/master/AWSLambda/covidall_db_add/lambda_function.py) tables to get today’s data into the AWS RDS PostgreSQL. As the [racing chart](https://www.planetdata.world/Pandemic/racing) and [bubbles](https://www.planetdata.world/Pandemic/bubbles) visualizations rely on these tables, so too do the visualizations which update in order to show relevant data. This is all the result of these self-sufficient functions. Side note: You’ll notice a third table exists in the database (uscounties); this was originally meant to be a dynamic table but it proved too much for both Lambda functions and Heroku.
